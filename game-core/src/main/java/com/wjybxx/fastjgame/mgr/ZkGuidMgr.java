...
@EventLoopSingleton
@NotThreadSafe
public class ZkGuidMgr implements GuidMgr {

    private static final Logger logger = LoggerFactory.getLogger(ZkGuidMgr.class);
    ...
    @Inject
    public ZkGuidMgr(CuratorMgr curatorMgr) {
    ...
    private void checkCache() throws Exception {
        ...
        if (guidSequence == Integer.MAX_VALUE) {
            ...
            curatorMgr.actionWhitLock(lockPath, this::incGuidIndex);
        }
    }
    ...
    private void init() throws Exception {
        ...
        curatorMgr.actionWhitLock(lockPath, () -> {
            if (!curatorMgr.isPathExist(guidIndexPath)) {
                // 初始化为1 并据为己有，序列化为字符串字节数组具有更好的可读性
                byte[] initData = GameUtils.serializeToStringBytes(1);
                curatorMgr.createNode(guidIndexPath, CreateMode.PERSISTENT, initData);
    ...
    private void incGuidIndex() throws Exception {
        byte[] oldData = curatorMgr.getData(ZKPathUtils.guidIndexPath());
        ...
        curatorMgr.setData(ZKPathUtils.guidIndexPath(), newData);
