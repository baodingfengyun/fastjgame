 ...
 */
@NotThreadSafe
public abstract class CuratorFacade {
    ...
    public CuratorFacade(CuratorFramework client) {
        this.client = client;
    }
     ...
     * <pre>
     *     {@code
     *         curatorMgr.lock(path);
     *         try{
     *             // do something
     *         } finally{
     *             curatorMgr.unlock(path);
     *         }
     *     }
     * </pre>
    ...
    public CloseableHandle watchChildren(String path, @Nonnull PathChildrenCacheListener listener) throws Exception {
        ...
        CloseableExecutorService watcherService = newClosableExecutorService();
        ...
        pathChildrenCache.getListenable().addListener(listener, listenerExecutor());
        // 避免外部忘记关闭
    ...
    /**
     * 创建一个executor用于{@link PathChildrenCache}后台拉取数据的线程
     */
    @Nonnull
    protected abstract CloseableExecutorService newClosableExecutorService();

    /**
     * @return 事件监听器回调方法的执行线程
     */
    @Nonnull
    protected abstract EventLoop listenerExecutor();
    ...
    public byte[] waitForNodeCreate(final String path) throws Exception {
        // 轮询虽然不雅观，但是正确性易保证
        byte[] result;
        while ((result = getDataIfPresent(path)) == null) {
            Thread.sleep(1000);
        }
        return result;
    }
