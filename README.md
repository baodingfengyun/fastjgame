...
拿交易系统举例，只有玩家在同一个场景时，这个操作才是允许的。比如：这个进程虽然承载了5000人，但是可能玩家的交易范围只有99人，单个场景内玩家数很有限。
> 3. 性能提升：它能提升较多的性能，能很好的利用多核的优势，每个线程都处于忙碌状态。
> 4. 可以ALL IN ONE（一个进程启动所有类型服务器），也可以独立部署（每个进程只启动一种类型服务器）。  
...
拿交易系统举例：如果一个进程整承载5000人，进程由5个场景world构成，每个world承载1000人，一个world代表一个小频道/分线，那么玩家的交易范围就是999人。
> 5. 性能提升：可以很好的提升性能，每个线程都是忙碌状态。
> 6. 关键问题：单个线程的表现不算好，如果游戏允许拆分较多的小频道/分线，那么这种模型才会有较大的优势。
> 7. 为保持编程模型的简单性，很多资源是以线程为资源分配的，存在一定程度的浪费(内存)。  
但是部分资源是可以以进程为单位分配的，比如我现在的架构中，网络资源是以进程为单位分配的，该进程内的所有线程都使用同一个NetEventLoopGroup，未来的DB也会这样。
> 8. 可以ALL IN ONE，也可以独立部署。
> 9. 它有着清晰的编程边界，编码难度基本与单线程架构一致，只有重点优化部分是多线程的，此外它可以很容易的退回到单线程模型。 
