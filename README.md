...
> 4. 数据共享：但是玩家只有在同一个场景下时，数据才是直接可用的，可能会限制部分功能实现（部分需求无法支持）。拿交易系统举例，只有玩家在同一个场景时，这个操作才是允许的。
...
> 4. 如果没有逻辑提取到多线程模块中，那么单个线程其实就对应了常见的多进程单线程架构中的单个进程。也就是说该模型是多进程单线程模型的一种改进。
> 5. 部分资源分配可以是以进程为单位的。比如我现在的架构中，网络资源是以进程为单位分配的，该进程内的所有线程都使用同一个NetEventLoopGroup，未来的DB也会这样。
> 6. 它有着清晰的编程边界，且很容易水平扩展，此外它可以很容易的退回到单线程模型，也可以把想要优化的部分提取到多线程模块中。
> 7. 可以ALL IN ONE，也可以独立部署。

> 1. 数据冗余：它对比service的方式，肯定有不少冗余的部分，但是又比单线程的方式少。
> 2. 性能提升：它对比service的方法，对cpu的利用率差一点，但比单线程的方式好。
