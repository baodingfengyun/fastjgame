...
1. 它将是一个分布式多进程多线程架构，它有着良好的多线程模型。  
...
> 1. 注意：每一个场景也是一个service，且玩家请求先由场景service处理。  
> 2. 数据冗余：它基本没有数据冗余，但是也可能存在一定的冗余（消除部分异步逻辑、简化部分功能开发）。  
> 3. 性能提升：它能提升较多的性能，能很好的利用多核的优势。  
> 4. 数据共享：但是玩家只有在同一个场景下时，数据才是直接可用的，可能会限制部分功能实现（涉及玩家之间交互的）。拿交易系统举例，只有玩家在同一个场景时，这个操作才是允许的。
> 5. 关键问题：由于拆分的太细，导致编程困难，开发效率很低，且安全性难以保证！！！
...
+ 按执行阶段拆分为单线程阶段、多线程阶段。服务器逻辑分为事件驱动和心跳驱动，其中事件处理必须是单线程的，而**心跳**有机会拆分为单线程执行阶段和多线程执行阶段。
对于MMO游戏而言，**场景**是一个很好的隔离单位！场景内的心跳逻辑大多只与场景内的数据有关，而场景心跳却是场景服务器耗时最多的部分，解决了该部分，性能瓶颈基本也就解决了。
> 1. 注意：该架构要求多线程阶段执行的逻辑一定不能访问(场景)外部数据。 eg：如果AI在多线程阶段，那么AI中的逻辑一定不能访问场景外部数据。
> 2. 执行流程：可参考FORK JOIN模式。主线程执行单线程心跳后，将后续的心跳逻辑提交为**任务**交给线程池执行（主线程也可以参与执行），然后阻塞到心跳任务全部完成(否则会导致线程安全问题)，然后主线程继续执行。
> 2. 数据共享：可以做到像单线程架构一样一个进程下的玩家数据都是可以互相访问的，玩家的体验也是最好的！且能支持远多于单线程架构的负载！这是一个很明显的优势。  
拿交易系统举例：如果一个进程表示一个大频道，那么允许该频道的玩家之间交易。这是其它模型不能办到的！
注意：处理业务逻辑的时候，涉及数据共享的逻辑，必须放在单线程阶段(或本身是线程安全的)！ 
> 3. 性能提升：对场景服务器的性能提升是非常显著的！如果其它服务器的心跳也以分阶段，那么也可以获得提升。
> 4. 关键问题：对于场景服务器，场景内的逻辑开发并不容易，需要专人维护！对于其它服务器，能安全的进行并行的逻辑可能不太多。
> 5. 这个模型我当初想过尝试，该模型还是很有诱惑力的，但是最终没进行（我可能还是有点不太想一个场景内既有单线程代码，又有多线程代码）。。。

#### 我的思路？
+ 我的思路最早是源于Netty的EventLoop，但是把它真正用到游戏中还是花了很长时间。它的模型大概如下：  
> 1. 一个进程分为多线程模块和单线程模块。
> 2. 多线程模块用于支持EventLoopGroup内的所有EventLoop，主要目的是为了减少系统资源（内存、网络、cpu）占用的。
只有那些占用特别多内存之类必须优化的部分才做到多线程模块中。eg：网络资源、DB资源、配置表、地图资源。  
> 3. 单线程模块即单个EventLoop，也是单个World；线程(world)之间是独立同构的；日常开发都是在单线程模块中。
> 4. 如果没有逻辑提取到多线程模块中，那么单个线程其实就对应了常见的**多进程单线程**架构中的单个进程。也就是说该模型是多进程单线程模型的一种改进。
> 5. 部分资源分配可以是以进程为单位的。比如我现在的架构中，网络资源是以进程为单位分配的，该进程内的所有线程都使用同一个NetEventLoopGroup，未来的DB也会这样。
> 6. 可以ALL IN ONE，也可以独立部署。
> 7. 它有着清晰的编程边界，且很容易扩展；此外它可以很容易的退回到单线程模型。 
> 8. 关键问题：不是很好描述，结论是：如果游戏允许拆分较多的小频道/分线，那么这种模型才会有较大的优势。

> 1. 数据冗余：它对比service的方式，肯定有不少冗余的部分，但是又比单线程的方式少。
> 2. 性能提升：可以很好的提升性能，对多数服务器有效，扩展较为容易。
> 3. 数据共享：玩家在同一个world的时候，数据才是直接可用的，基本和单线程架构是一样的。
> 4. 编码难度：编码难度基本与单线程架构一致，只有重点优化部分是多线程的。
