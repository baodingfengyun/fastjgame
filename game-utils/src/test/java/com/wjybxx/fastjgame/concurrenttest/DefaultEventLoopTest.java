...
public class DefaultEventLoopTest {
	...
	public static void main(String[] args) {
		...
		future3.addListener((listenableFuture) -> {
			System.out.println("1 onComplete, thread " + Thread.currentThread().getName() + ",result = " + listenableFuture.tryGet());
		}, GlobalEventLoop.INSTANCE);

		future3.addListener((listenableFuture) -> {
			System.out.println("2 onComplete, thread " + Thread.currentThread().getName() + ",result = " + listenableFuture.tryGet());
		});

		future3.addListener((listenableFuture) -> {
			System.out.println("3 onComplete, thread " + Thread.currentThread().getName() + ",result = " + listenableFuture.tryGet());
			// 已完成到时候添加监听
			future3.addListener(listenableFuture2 -> {
				System.out.println("4 onComplete, thread " + Thread.currentThread().getName() + ",result = " + listenableFuture2.tryGet());
			}, GlobalEventLoop.INSTANCE);
		});
