 ...
 *
 * <h3>监听器性能</h3>
 * 使用合适的{@link #workerExecutor}将有助于减小通知监听器的开销。
 * {@link #workerExecutor}应该为使用监听器最多的线程。
 *
 * <p>
...
public class DefaultPromise<V> extends AbstractPromise<V> {
    ...
    public DefaultPromise(@Nonnull Executor workerExecutor) {
    ...
    /**
     * 用于{@link FluentFuture#get()} 和{@link FluentFuture#get(long, TimeUnit)}抛出失败异常。
     *
     * @param cause 任务失败的原因
    ...
    private void checkDeadlock() {
        if (EventLoopUtils.inEventLoop(workerExecutor)) {
            throw new BlockingOperationException("Blocking await may caught deadlock");
        }
    }
    ...
    protected <U> AbstractPromise<U> newIncompletePromise() {
        return new DefaultPromise<>(workerExecutor);
    }
