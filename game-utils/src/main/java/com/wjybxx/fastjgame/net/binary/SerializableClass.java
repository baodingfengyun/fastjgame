 ...
 * 1. 如果是枚举，必须实现{@link IndexableEnum}，并提供静态非private的{@code forNumber(int)}方法 - 也就是按照protoBuf的枚举格式来。
 * 2. 如果是实现了{@link IndexableEnum}的类，也必须提供静态非private的{@code forNumber(int)}方法。
 * 3. 如果是实现了{@link IndexableObject}的类，必须提供静态非private的{@code forIndex(Object)}方法。
 ...
 *
 * <h3>性能</h3>
 * 生成的类中，能使用普通方法调用的，就会使用普通方法调用(构造方法、取值、设值方法)，如果字段是final的，或没有相应的设值方法，就会使用反射。
