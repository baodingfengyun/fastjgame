...
public class CompositeDynamicTimeRange implements DynamicTimeRange {
	...
	public TimeRange triggeringTimeRange(long timeMs) {
		...
		Optional<TimeRange> min = children.stream()
				...
				.min(this::compareNextTriggerTimeRange);
		// 可能一个都没有触发，因此结果可能不存在
	...
	public TimeRange preTriggerTimeRange(long timeMs) {
		...
		Optional<TimeRange> min = children.stream()
				...
				.filter(Objects::nonNull)
				.max(this::comparePreTriggerTimeRange);
		// 可能都没有上一个触发点
		return min.orElse(null);
	}

	private int compareNextTriggerTimeRange(TimeRange a, TimeRange b){
		// startTime越小越靠前
	...
	private int comparePreTriggerTimeRange(TimeRange a, TimeRange b){
		// endTime越小越靠前
