 ...
 * 表示订阅一个事件。
 * 注意：
 * 1.使用该注解的方法要么有一个参数，要么有两个参数，否则编译报错。
 * <p>
 * <b>当参数格式为1时</b>：
 * 第一个参数：订阅的事件，不可以是基本类型，因为发布事件的时候会封装为Object，基本类型会被装箱，会导致问题。
 * <p>
 * <b>当参数个数为2时</b>
 * 第一个参数: 为抛出事件时的上下文，可以是基本类型，生成的代码会自动拆装箱以解决重载问题。
 * 第二个参数：订阅的事件，不可以是基本类型，因为发布事件的时候会封装为Object，基本类型会被装箱，会导致问题。
 * <p>
 * 2.如果期望订阅多个事件，请使用{@link #subEvents()}声明关注的其它事件。
 * 3.方法不能是private - 至少是包级访问权限。
 * <p>
